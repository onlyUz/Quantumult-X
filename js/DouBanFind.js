...
`
🤖 本脚本转换自：https://greasyfork.org/scripts/329484-豆瓣资源下载大师-1秒搞定豆瓣电影-音乐-图书下载/code/豆瓣资源下载大师：1秒搞定豆瓣电影|音乐|图书下载.user.js。
...
[MITM]
hostname=movie\.douban\.com/.*, music\.douban\.com/.*, book\.douban\.com/.*, bangumi\.moe/search/title.*, desitorrents\.tv/.*, www\.dingzi66\.com/q.*, www\.x5v\.net/.*, 

🔘 Quantumult X
[rewrite_local]
movie\.douban\.com/.* url script-response-body DouBanFind.js
music\.douban\.com/.* url script-response-body DouBanFind.js
book\.douban\.com/.* url script-response-body DouBanFind.js
bangumi\.moe/search/title.* url script-response-body DouBanFind.js
desitorrents\.tv/.* url script-response-body DouBanFind.js
www\.dingzi66\.com/q.* url script-response-body DouBanFind.js
www\.x5v\.net/.* url script-response-body DouBanFind.js
...
try {
    ...
    if (/<\/html>|<\/body>/.test(body)) {
        body = body.replace('</body>', `<script src=https://cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.min.js></script>
<script src=https://greasyfork.org/scripts/368137-encodeToGb2312/code/encodeToGb2312.js?version=601683></script>
...
<div class="rating_more" style="display:none"></div>\`); // 修复部分情况\$("div.rating_betterthan")不存在情况
    ...
    \$("#drdm_setting_btn").click(function () {
        ...
        console.log("[油猴脚本] 豆瓣资源下载大师：1秒搞定豆瓣电影|音乐|图书下载.user.js 注入成功!");
    }
    $done({body});
} catch (err) {
    console.log("[油猴脚本] 豆瓣资源下载大师：1秒搞定豆瓣电影|音乐|图书下载.user.js 执行失败!\n" + err);
    $done({});
}
